...
<div class="page-hero">
    <div class="container">
        <div class="breadcrumb">
            ...
            <a href="index.php?category=<?= urlencode($event['category']) ?>"><?= htmlspecialchars($event['category']) ?></a>
            <span>/</span><?= htmlspecialchars($event['title']) ?>
        </div>
        <h1><?= htmlspecialchars($event['title']) ?></h1>
        <p><?= $icon . ' ' . htmlspecialchars($event['category']) ?> &nbsp;|&nbsp; <?= htmlspecialchars($event['location']) ?></p>
    </div>
</div>
    ...
    <div class="container">
        <div class="event-detail-grid">
                ...
                <div class="event-info mt-3">
                    ...
                    <ul class="event-meta-list mt-2">
                        ...
                        <li class="event-meta-row">
                            <div class="meta-icon">&#129686;</div>
                            <div class="meta-text">
                                <strong style="color:<?= $low ? '#ef4444' : '#10b981' ?>">
            ...
            <aside class="booking-sidebar">
                ...
                <div class="sidebar-seats <?= $low ? 'low' : '' ?>">
                ...
                <div class="sidebar-info-row" style="margin-top:.5rem;padding-top:.75rem;border-top:1px solid var(--border)">
                    <span>Price per ticket</span>
                    <strong style="color:var(--accent)">EGP <?= number_format((float)$event['price'], 2) ?></strong>
                </div>
                <div style="margin-top:1.5rem">
                    <?php if ($event['available_seats'] == 0): ?>
                        <button class="btn btn-ghost btn-full" disabled>Sold Out</button>
                    <?php else: ?>
                        <a href="<?= htmlspecialchars($book_href) ?>" class="btn btn-primary btn-full btn-lg">
                            &#127917; Book Now
                        </a>
                        <?php if (!isset($_SESSION['user_id'])): ?>
                            <p class="text-center text-muted mt-1" style="font-size:.82rem;">
                                <a href="login.php" style="color:var(--accent);font-weight:600;">Log in</a> to book tickets.
                            </p>
                        <?php endif; ?>
                    <?php endif; ?>
                </div>
